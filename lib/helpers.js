 ...
 * //TODO: Combine these three functions into one walk of the object graph. It currently walks the graph 3 times.
 * @param {*} meta Metadata
 * @return {*}
 */
exports.prepareMetaData = meta => {
  ...
  //Use lodash's cloneDeepWith so we can handle dates, ObjectIDs, etc nested in the object graph.
  meta = cloneDeepWith(meta, cloneDeepCustomizer);
  return meta;
};
...
function cloneDeepCustomizer(value, key, obj) {
  ...
  if (value instanceof Error) {
    return assignIn({}, cloneDeepWith(value, cloneDeepCustomizer));
  }
